...
class testfunc {
	...
	//         init population, g_best,  popn_size	 dim  , benchmark_fn
	void test_func(double[] x, double[] f, int nx, int mx,int func_num) throws Exception{
		...
		if (ini_flag==0) /* initiailization*/
		{
			...
			String pathPrefix = "./EAM/input_data";
			if(!Benchmark.isCmd)
				pathPrefix = pathPrefix.replace("./","./out/production/EAM/");

			File fpt = new File(pathPrefix + "/M_D"+nx+".txt");//* Load M data *
			...
			fpt=new File(pathPrefix +"/shift_data.txt");
